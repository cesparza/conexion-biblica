...
# La ruta sale del propio archivo, como en gen_creencias.py. Estaba quemada a
# `~/mnt/Iglesia/...`, que era donde el puente montaba la carpeta en su momento:
# el dia que el montaje cambio, el generador dejo de escribir y fallo con un
# FileNotFoundError que no dice nada de lo que de verdad pasa.
RAIZ = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..')
SALIDA = os.path.normpath(os.path.join(RAIZ, 'fuente', 'cartilla.txt'))
...
L.append('    Es un error de imprenta, no una variante: el libro de las 28')
L.append('    (capitulo 14, pagina 200) si trae la declaracion de la 14, y habla')
L.append('    de la unidad y no del remanente. La app la muestra desde el libro,')
L.append('    marcada como tal; aqui no se escribe, porque la cartilla no la')
L.append('    trae.')
L.append('')
...
for n, nom, a, b, intro in datos.DOCTRINAS:
    ...
    for num in range(a, b + 1):
        ...
        L.append('')
        # SOLO LO QUE ES DE LA CARTILLA. Una declaracion que salio del libro no
        # se escribe aqui aunque exista: este archivo es la transcripcion
        # literal de la cartilla, y es contra el que tools/cartilla.js mide.
        if d['decl'] and d['fuente'] == 'cartilla':
            L.append(d['decl'])
        else:
            L.append('(SIN DECLARACION EN LA CARTILLA: %s. Ver la nota de arriba.)' % d['fuente'])
        L.append('')
